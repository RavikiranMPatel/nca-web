    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          <button onClick={() => navigate("/home")} className="p-1.5 text-gray-500 hover:bg-gray-100 rounded-lg transition">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div>
            <h1 className="text-base font-bold text-gray-900">🏏 Cricket Stats</h1>
            <p className="text-xs text-gray-400">Academy leaderboards</p>
          </div>
          {/* Period toggle */}
          <div className="ml-auto flex bg-gray-100 rounded-xl p-1 gap-1">
            <button onClick={() => setPeriod("alltime")}
            ...
            <button onClick={() => setPeriod("season")}
      ...
      <div className="max-w-4xl mx-auto px-4 pt-5 space-y-6">
        {loading ? (
          <div className="flex justify-center py-16">
            <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: `${primaryColor}40`, borderTopColor: primaryColor }} />
          </div>
        ) : (
          <>
            {/* Top Performers Widget */}
            {topPerformers && (topPerformers.topBatters.length > 0 || topPerformers.topBowlers.length > 0) && (
                ...
                {topPerformers.topBatters[0] && (
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="px-4 py-2.5 border-b border-gray-50 flex items-center gap-2"
                      ...
                      <span className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Top Batter</span>
                    </div>
                    <div className="p-4">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          { label: "Runs", value: period === "season" ? topPerformers.topBatters[0].runs : topPerformers.topBatters[0].runs },
                ...
                {topPerformers.topBowlers[0] && (
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="px-4 py-2.5 border-b border-gray-50 flex items-center gap-2"
                      style={{ background: `${primaryColor}08` }}>
                      <span className="text-lg">⚾</span>
                      <span className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Top Bowler</span>
                    </div>
                    <div className="p-4">
                      <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
              <button onClick={() => setSubTab("batting")}
              ...
              <button onClick={() => setSubTab("bowling")}
              ...
              battingList.length === 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-4 py-3 border-b border-gray-50">
                    <h3 className="text-sm font-semibold text-gray-700">Batting Leaderboard</h3>
                  </div>
                  <div className="divide-y divide-gray-50">
                    {battingList.map((p, i) => (
                      <div key={p.playerPublicId} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition active:bg-gray-100 cursor-pointer"
                        onClick={() => navigate(`/players/${p.playerPublicId}/profile`)}>
                        {/* Rank */}
                        ...
                        {/* Avatar */}
                        <div className="w-9 h-9 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center text-sm font-bold text-white"
                          style={{ background: `${primaryColor}` }}>
                          {p.photoUrl ? <img src={p.photoUrl} alt="" className="w-full h-full object-cover" /> : p.playerName.charAt(0)}
                        </div>
                        {/* Name */}
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-semibold text-gray-900 truncate">{p.playerName}</div>
                          <div className="text-xs text-gray-400">{p.matches} matches · HS {p.highScore}</div>
                        </div>
                        {/* Key stats */}
                        <div className="flex items-center gap-3 text-right flex-shrink-0">
                          <div>
                            <div className="text-base font-black" style={{ color: primaryColor }}>{period === "season" ? p.runs : p.runs}</div>
                            <div className="text-xs text-gray-400">runs</div>
                          </div>
                          {period === "alltime" && (
                            <div className="hidden sm:block">
                              <div className="text-sm font-semibold text-gray-700">{p.average}</div>
            ...
            {subTab === "bowling" && (
              bowlingList.length === 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-4 py-3 border-b border-gray-50">
                    <h3 className="text-sm font-semibold text-gray-700">Bowling Leaderboard</h3>
                  </div>
                  <div className="divide-y divide-gray-50">
                    {bowlingList.map((p, i) => (
                      <div key={p.playerPublicId} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition active:bg-gray-100 cursor-pointer"
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-semibold text-gray-900 truncate">{p.playerName}</div>
                          <div className="text-xs text-gray-400">{p.matches} matches · Best {p.bestFigures}</div>
                        </div>
                        <div className="flex items-center gap-3 text-right flex-shrink-0">
                          <div>
                            <div className="text-base font-black" style={{ color: primaryColor }}>{period === "season" ? p.wickets : p.wickets}</div>
                            <div className="text-xs text-gray-400">wkts</div>
                          </div>
                          {period === "alltime" && (
                            <div className="hidden sm:block">
                              <div className="text-sm font-semibold text-gray-700">{p.economy}</div>
